...
// 四分之一周期查表，数值为 sin(angle) * 10000；保留原 200 个采样点。
static constexpr int16_t SINE_TABLE[200] =
{
...
/**
 * @brief 用原查表与舍入规则近似计算正弦
 *
 * @param angle 角度，范围 0 至 2PI
 *
 * @return 正弦近似值
 */
float _sin(float angle)
{
    if(angle < _PI_2)
    {
        return 0.0001f * SINE_TABLE[_round(126.6873f * angle)];
    }
    else if(angle < _PI)
    {
        return 0.0001f * SINE_TABLE[398 - _round(126.6873f * angle)];
    }
    ...
    {
        return -0.0001f * SINE_TABLE[796 - _round(126.6873f * angle)];
    }
}
 ...
 * @param angle 任意弧度角
 *
 * @return 归一化角度，范围 0 至 2PI
 */
float _normalizeAngle(float angle)
{
    float normalized = fmod(angle, _2PI);
