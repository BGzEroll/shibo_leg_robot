 ...
 *
 * @param requested 是否请求关闭 WiFi 省电
 *
 * @return 当前允许设置的 WiFi 省电模式
 */
static wifi_ps_type_t select_wifi_sleep_type(bool requested)
{
    bool bluetooth_enabled =
        esp_bt_controller_get_status() == ESP_BT_CONTROLLER_STATUS_ENABLED;
    if(requested && !bluetooth_enabled){return WIFI_PS_NONE;}
    return WIFI_PS_MIN_MODEM;
}

/**
 * @brief 在 WiFi 维护周期中应用最新低延迟请求
 */
static void apply_low_latency_request()
{
    ...
    if(!requested &&
       (!restore_at_ms || (int32_t)(now_ms - restore_at_ms) < 0)){return;}
 ...
 * @return 连接成功时返回 true
 */
static bool connect_station(const String &ssid, const String &password,
    wifi_mode_t mode, uint32_t timeout_ms)
{
    WiFi.mode(mode);
 ...
 */
void hw::wifi::set_low_latency_mode(bool enabled)
{
