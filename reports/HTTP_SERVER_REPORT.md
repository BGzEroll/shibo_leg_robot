...
AP 配网模式和 Station 模式都使用同一个 Console。AP 配网模式下只显示 WiFi 设置和蓝牙设置入口；后续如果新增更多上位机模块，应在 `console_html()` 中按 `wifi_dev::config_portal_active()` 控制可见性，避免 AP 配网状态暴露无关入口。
...
6. 当前页面已经能覆盖最需要的实机操作：配置 WiFi、扫描 BLE、选择 Xbox 手柄。
...
6. `/api/xbox/select` 只校验地址长度为 17，没有校验 `xx:xx:xx:xx:xx:xx` 的十六进制格式。
7. 页面中的 SSID 和 BLE 名称做了 JSON 转义，但 HTML 模板拼接仍比较轻量，未来如果输入源更复杂，最好增加 HTML escape。
8. `esp_http_server::task_entry()` 的参数 `arg` 当前未使用，符合项目里“不写 `(void)arg;`”的风格，但编译选项如果未来打开未使用参数警告，可能需要统一处理。
